...
(Reality / hysteria2) и откатываясь на собственный forward-secret канал magnetgate. Цензор не видит
ни централизованного брокера, ни единственного «подозрительного» адреса для простой блокировки:
рандеву идёт через BitTorrent Mainline DHT **и** пул Nostr-релеев, а дата-плоскость выглядит как
обычный TLS / QUIC к реальному сайту.
...
| **hysteria2** | QUIC (UDP/443) + salamander obfs | альтернатива | хороша на потерях/мобиле; пока `insecure` TLS (см. ограничения) |
...
Exit публикует один и тот же sealed-offer в оба канала, так что discovery переживает блокировку/шейпинг любого из них:

- **Mainline DHT (BEP 44)** — mutable-запись по ключу ed25519 из PSK; republish каждые 5 мин. Ставьте
...
- **Харденинг** — exit/DHT/sing-box работают непривилегированно в песочницах systemd; PSK не попадает
  ...
  SHA-256; `npm ci` для воспроизводимых установок.

Известные ограничения: hysteria2 пока с `insecure` TLS (трафик шифруется, клиент аутентифицируется
паролем+obfs, но самоподписанный серверный cert не валидируется) — чтобы offer влезал в лимит DHT
1000 б; Reality аутентифицирован полностью, пиннинг cert для hy2 — задача Фазы 3. Обфускация нативного
...
- **Пиннинг cert для hy2** — доставлять серверный cert через Nostr-offer (без лимита размера), убрав
  `insecure` для hysteria2.
